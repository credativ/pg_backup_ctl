...
assert pgbc -L 90M -M "/dev/${id_vg}"/"${id_lv}" -n "pgsnap${id_number}" -N "data" -o noexec create-lvmsnapshot
assert grep -Eq '^[^ ]+ '"${test_archive_dir}"'/lvm_snapshot [^ ]+ ([^ ]*,)?noexec[, ]' /proc/mounts
